import {
  ...
  /**
   * This function was taken from https://github.com/codeuniversity/antd-weekly-calendar 
   * under a MIT licence
  ...
  export const getDayHoursEvents = <T extends GenericEvent>(
    ...
    for (let i = 0; i < ROW_AMOUNT; i++) {
      ...
      events.push({
        ...
        Sunday: weekObject?.sunday && weekObject?.sunday.filter(e => isSameHour(e.startTime, add(hour, { days: 0 }))),
        Monday: weekObject?.monday && weekObject?.monday.filter(e =>  isSameHour(e.startTime, add(hour, { days: 1 }))),
        Tuesday: weekObject?.tuesday && weekObject?.tuesday.filter(e => isSameHour(e.startTime, add(hour, { days: 2 }))),
        Wednesday: weekObject?.wednesday && weekObject?.wednesday.filter(e => isSameHour(e.startTime, add(hour, { days: 3 }))),
        Thursday: weekObject?.thursday &&  weekObject?.thursday.filter(e =>isSameHour(e.startTime, add(hour, { days: 4 }))),
        Friday: weekObject?.friday && weekObject?.friday.filter(e => isSameHour(e.startTime, add(hour, { days: 5 }))),
        Saturday: weekObject?.saturday && weekObject?.saturday.filter(e => isSameHour(e.startTime, add(hour, { days: 6 })))  
      });
    }
    return events;
  };
